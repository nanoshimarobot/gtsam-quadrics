...
int main(void) {
    ...
    // define key poses
    vector<Pose3> poses;
    poses.push_back(Pose3(Rot3(), Point3(0.0, 0.0, 0.0)));
    poses.push_back(Pose3(Rot3(), Point3(0.0, 0.0, 0.0)));

    for(size_t i = 0; i < poses.size(); ++i){
        poses[i].print("pose" + std::to_string(i) + " : \n");
    }

    // define quadrics
    vector<ConstrainedDualQuadric> quadrics;
    quadrics.push_back(ConstrainedDualQuadric(Pose3(Rot3(), Point3(0.5, 2.5, 0.5)), Vector3(0.3, 0.4, 0.5)));
    ...
    for (unsigned i = 0; i < noisyTrajectory.size()-1; i++) {
    ...
    for (unsigned i = noisyTrajectory.size() - 1; i < noisyTrajectory.size(); i++) {
        Key poseKey(Symbol('x', i));
    ...
    // initialEstimate.print("initial_estimates");
    // graph.print();
